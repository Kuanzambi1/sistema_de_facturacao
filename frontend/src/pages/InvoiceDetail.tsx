...
export default function InvoiceDetail() {
  ...
  const isConvertible = (invoice?.documentType === "OR" || invoice?.documentType === "PP" || invoice?.documentType === "FP") && invoice?.status === "emitida";
  const { data: ftSeries } = trpc.series.list.useQuery({ documentType: "FT" }, { enabled: isConvertible });
  const [convertOpen, setConvertOpen] = useState(false);
  const [selectedFtSeriesId, setSelectedFtSeriesId] = useState<number | undefined>();
  const { user: currentUser } = useAuth();
  const isAdmin = currentUser?.role === "admin";

  // Auto-select first active FT series
  useEffect(() => {
    if (ftSeries && ftSeries.length > 0 && !selectedFtSeriesId) {
      const active = ftSeries.find((s) => s.isActive) ?? ftSeries[0];
      if (active) setSelectedFtSeriesId(active.id);
    }
  }, [ftSeries, selectedFtSeriesId]);
  ...
  const convertQuotation = trpc.invoices.convertQuotation.useMutation({
    onSuccess: (inv) => {
      ...
      utils.invoices.get.invalidate({ id });
      const label = (invoice?.documentType === "PP" || invoice?.documentType === "FP") ? "Factura Proforma convertida" : "Orçamento convertido";
      toast.success(`${label} na factura ${inv.fullNumber}!`);
  ...
  const deleteInvoice = trpc.invoices.delete.useMutation({
    onSuccess: () => { utils.invoices.list.invalidate(); toast.success("Documento eliminado!"); navigate("/documentos"); },
    onError: (e) => toast.error(e.message),
  });
    ...
    try {
      ...
      if (company?.logoUrl) {
        if (company.logoUrl.startsWith("data:")) {
          ...
          try {
            const resp = await fetch(company.logoUrl);
      ...
      if (logoDataUrl) {
        try {
          doc.addImage(logoDataUrl, "PNG", mx, y - 10, 18, 18);
          logoH = 20;
        } catch { /* ignore */ }
      }
      drawText((company?.name || "Sua Empresa").toUpperCase(), mx + logoH, y, 13, "helvetica", "bold");
      y += 6;
      ...
      const summaryRows: Array<{ label: string; value: string; bold?: boolean }> = [
        { label: "Total sem imposto:", value: formatKz(grossTotal) },
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        if (invoice.documentType === "PP" || invoice.documentType === "FP" || invoice.documentType === "OR") {
          ...
          doc.text("ESTE DOCUMENTO NÃO SERVE DE FACTURA — SEM VALOR CONTABILÍSTICO", 105, pageH - 11, { align: "center" });
        } else {
    ...
    <div ref={invoiceRef} className="p-6 space-y-5 animate-fade-in-up max-w-4xl mx-auto">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {isConvertible && (
            <Button size="sm" className="gap-1.5 bg-violet-600 hover:bg-violet-700 text-white font-medium shadow-sm" onClick={() => setConvertOpen(true)}>
          ...
          {(() => {
            ...
            const hasNC = (invoice as any).ncCount > 0;
            const canDelete = isPaid ? hasNC : (isAdmin || hasNC);
            return canDelete ? (
              <Button variant="outline" size="sm" className="gap-1.5 text-destructive border-destructive hover:bg-destructive/10"
                onClick={() => { if (confirm("Eliminar este documento permanentemente? Esta acção não pode ser revertida.")) deleteInvoice.mutate({ id }); }}>
      ...
      {/* Dialog converter em factura */}
      <Dialog open={convertOpen} onOpenChange={setConvertOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Repeat className="h-5 w-5 text-violet-600" />
              Converter em Factura Definitiva (FT)
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              Esta acção irá transformar este documento numa <strong>Factura (FT)</strong> oficial com valor fiscal e contabilístico, assinada digitalmente com hash SHA-256 e numerada sequencialmente.
            </p>

            <div className="rounded-lg bg-violet-50/70 border border-violet-200/70 p-3 space-y-1.5 text-xs text-violet-950">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Documento de Origem:</span>
                <span className="font-semibold">{DOCUMENT_TYPE_LABELS[invoice.documentType]} {invoice.fullNumber}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Cliente:</span>
                <span className="font-semibold">{invoice.clientName || "Consumidor Final"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Total:</span>
                <span className="font-bold text-sm text-foreground">{formatCurrency(totalAmount)}</span>
              </div>
            </div>

            <div className="space-y-1.5">
              <Label>Série da Factura *</Label>
              <Select value={selectedFtSeriesId ? String(selectedFtSeriesId) : undefined} onValueChange={(v) => setSelectedFtSeriesId(Number(v))}>
                <SelectTrigger><SelectValue placeholder="Seleccione a série FT..." /></SelectTrigger>
                <SelectContent>
                  {ftSeries?.filter((s) => s.isActive).map((s) => (
                    <SelectItem key={s.id} value={String(s.id)}>
                      {s.code} — {s.name} (Próximo nº: {s.lastNumber + 1})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {(!ftSeries || ftSeries.length === 0) && (
                <p className="text-xs text-destructive">Nenhuma série FT activa disponível. Crie uma série nas Configurações.</p>
              )}
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!selectedFtSeriesId || convertQuotation.isPending}
              onClick={() => {
                if (!selectedFtSeriesId) return;
                convertQuotation.mutate({ quotationId: id, seriesId: selectedFtSeriesId });
              }}
            >
              {convertQuotation.isPending ? "A converter..." : "Confirmar e Emitir FT"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {/* Banners Informativos de Estado e Conversão */}
      {invoice.status === "convertida" && invoice.convertedInvoiceId && (
        <div className="p-4 bg-violet-50 border border-violet-200 rounded-lg flex items-center justify-between gap-3 text-sm text-violet-900">
          <div className="flex items-center gap-2.5">
            <span className="text-lg">✅</span>
            <span>
              Este documento foi <strong>convertido em Factura definitiva</strong>.
            </span>
          </div>
          <Button size="sm" variant="outline" className="border-violet-300 text-violet-700 hover:bg-violet-100" onClick={() => navigate(`/documentos/${invoice.convertedInvoiceId}`)}>
            Ver Factura Emitida →
          </Button>
        </div>
      )}

      {invoice.relatedInvoiceId && invoice.relatedInvoiceNumber && invoice.documentType === "FT" && (
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between gap-3 text-sm text-blue-900">
          <div className="flex items-center gap-2.5">
            <span className="text-lg">📄</span>
            <span>
              Factura emitida a partir do documento <strong>{invoice.relatedInvoiceNumber}</strong>.
            </span>
          </div>
          <Button size="sm" variant="outline" className="border-blue-300 text-blue-700 hover:bg-blue-100" onClick={() => navigate(`/documentos/${invoice.relatedInvoiceId}`)}>
      ...
      {(invoice.documentType === "PP" || invoice.documentType === "FP" || invoice.documentType === "OR") && invoice.status === "emitida" && (
        <div className="p-4 bg-amber-50/90 border border-amber-200 rounded-lg flex items-center justify-between gap-3 text-xs sm:text-sm text-amber-900">
          <div className="flex items-center gap-2.5">
            ...
            <span>
              <strong>{DOCUMENT_TYPE_LABELS[invoice.documentType]}:</strong> Este documento tem finalidade meramente orçamental/previsional e não possui eficácia fiscal.
            </span>
          </div>
          <Button size="sm" className="bg-violet-600 hover:bg-violet-700 text-white shrink-0" onClick={() => setConvertOpen(true)}>
            <Repeat className="h-3.5 w-3.5 mr-1" />Converter em Factura
          </Button>
        </div>
      )}
      ...
      <div className="bg-white border border-border rounded-lg shadow-sm overflow-hidden">
        ...
        <div className="p-6 flex flex-col sm:flex-row justify-between gap-6 border-b border-border">
          <div className="min-w-0 flex flex-col sm:flex-row items-start gap-4">
            {company?.logoUrl && (
              <img src={company.logoUrl} alt="Logótipo" className="h-16 w-16 sm:h-20 sm:w-20 object-contain rounded border border-border shrink-0" />
            )}
            <div className="min-w-0 space-y-0.5">
              ...
              {[company?.city, company?.province].filter(Boolean).length > 0 && (
                <p className="text-xs text-muted-foreground">{[company?.city, company?.province].filter(Boolean).join(", ")}</p>
              )}
              {[company?.phone && `Tel: ${company.phone}`, company?.email && `E-mail: ${company.email}`].filter(Boolean).length > 0 && (
                <p className="text-xs text-muted-foreground">{[company?.phone && `Tel: ${company.phone}`, company?.email && `E-mail: ${company.email}`].filter(Boolean).join("   |   ")}</p>
              )}
              {company?.nif && <p className="text-xs font-semibold text-foreground">Contribuinte: {company.nif}</p>}
            </div>
          </div>
          <div className="shrink-0 border-2 border-slate-800 rounded-md px-5 py-3 text-center min-w-56">
            ...
            {(invoice.documentType === "PP" || invoice.documentType === "FP" || invoice.documentType === "OR") && (
              <p className="text-[10px] font-bold text-amber-700 uppercase tracking-wide mt-1">Este documento não serve de factura</p>
            )}
