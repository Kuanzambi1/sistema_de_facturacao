...
import { Plus, Search, Eye, Download, FileX, Filter, FileText, Trash2, Repeat } from "lucide-react";
...
export default function Invoices() {
  ...
  const createPayment = trpc.payments.create.useMutation({
    onSuccess: () => { utils.invoices.list.invalidate(); toast.success("Pagamento registado!"); },
    onError: (e) => toast.error(e.message),
  });

  const deleteInvoice = trpc.invoices.delete.useMutation({
    onSuccess: () => { utils.invoices.list.invalidate(); toast.success("Documento eliminado!"); },
    onError: (e) => toast.error(e.message),
  });
                ...
                <tr key={inv.id}>
                  ...
                  <td>
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {(inv.documentType === "PP" || inv.documentType === "FP" || inv.documentType === "OR") && inv.status === "emitida" ? (
                        <Link href={`/documentos/${inv.id}`}>
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-violet-600 hover:text-violet-700 hover:bg-violet-50" title="Converter em Factura (FT)">
                            <Repeat className="h-3.5 w-3.5" />
                          </Button>
                        </Link>
                      ) : inv.status === "emitida" ? (
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-emerald-600" title="Marcar como paga"
                          onClick={() => createPayment.mutate({ invoiceId: inv.id, amount: Number(inv.totalAmount), paymentDate: new Date(), method: "transferencia" })}>
                      ...
                      {inv.status !== "paga" && inv.status !== "parcialmente_paga" && (
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-destructive" title="Eliminar"
                          onClick={() => { if (confirm("Eliminar este documento permanentemente? Esta acção não pode ser revertida.")) deleteInvoice.mutate({ id: inv.id }); }}>
