...
async function sendInvoiceEmail(req: any, invoice: any, tenantId: number) {
  ...
  try {
    ...
    const items = await db.getInvoiceItems(tenantId, invoice.id);
        const pdfBuffer = await generateInvoicePdf({ ...invoice, items }, company ?? ({} as any));
...
export const appRouter = router({
  ...
  tenant: router({
    ...
    usage: protectedProcedure.query(async ({ ctx }) => {
      ...
      const plan = PLANS[(tenant?.plan ?? "gratis") as PlanId];
      const used = await db.countMonthlyDocuments(tenantId);
      return { plan: tenant?.plan ?? "gratis", planLabel: plan.label, used, limit: plan.monthlyDocs, maxUsers: plan.maxUsers, recurring: plan.recurring, portal: plan.portal, status: tenant?.status ?? "trial", billingCycle: (tenant as any)?.billingCycle ?? "mensal" };
    }),
    updatePlan: adminProcedure
      .input(z.object({ plan: z.enum(["gratis", "pro", "escritorio"]), billingCycle: z.enum(["mensal", "trimestral", "semestral", "anual"]).optional() }))
      .mutation(async ({ input, ctx }) => {
        const tenantId = requireTenant(ctx.user);
        const updateData: any = { plan: input.plan };
        if (input.billingCycle) updateData.billingCycle = input.billingCycle;
        const tenant = await db.updateTenant(tenantId, updateData);
        await db.addAuditLog(tenantId, { userId: ctx.user.id, userName: ctx.user.name ?? null, action: "plano", entityType: "tenant", entityId: tenantId, entityLabel: `${input.plan}${input.billingCycle ? `/${input.billingCycle}` : ""}` });
        return tenant;
      }),
  }),
  ...
  company: router({
    ...
    uploadLogo: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const dataUrl = `data:image/${mime};base64,${input.fileBase64}`;
  ...
  products: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const tenantId = requireTenant(ctx.user);
        const code = input.code || (await db.getNextProductCode(tenantId, input.type));
        const created = await db.createProduct(tenantId, { ...input, code } as any);
  ...
  invoices: router({
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db.deleteInvoice(tenantId, input.id);
        await db.addAuditLog(tenantId, { userId: ctx.user.id, userName: ctx.user.name ?? null, action: "eliminar", entityType: "invoice", entityId: input.id, entityLabel: invoice.fullNumber ?? null });
        return { success: true };
      }),
    ...
    convertQuotation: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!invoice) throw new TRPCError({ code: "BAD_REQUEST", message: "Não foi possível converter o documento." });
        ...
        await db.applyStockMovementsForInvoice({
          ...
          createdBy: ctx.user.id,
        }).catch((e) => console.error("[Stock] movement on conversion failed:", e));

        await agtSubmitInvoice(tenantId, invoice.id).catch((e) => console.error("[AGT] submit failed:", e));
        await sendInvoiceEmail(ctx.req, invoice, tenantId).catch((e) => console.error("[Email] send failed:", e));
        ...
        await db.addAuditLog(tenantId, {
          ...
          entityType: "invoice",
          entityId: invoice.id,
          entityLabel: invoice.fullNumber ?? null,
        });
        return invoice;
      }),
  ...
  portal: router({
    ...
    invoice: publicProcedure
      .input(z.object({ token: z.string().min(1), invoiceId: z.number() }))
      .query(async ({ input }) => {
        const client = await db.getClientByPortalToken(input.token);
        if (!client) throw new TRPCError({ code: "NOT_FOUND", message: "Link inválido" });
        const invoice = await db.getInvoiceById(client.tenantId, input.invoiceId);
        if (!invoice || invoice.clientId !== client.id) throw new TRPCError({ code: "NOT_FOUND", message: "Documento não encontrado" });
        const company = await db.getCompany(client.tenantId);
  ...
  inventory: router({
    ...
    stockAlerts: protectedProcedure.query(async ({ ctx }) => {
      const tenantId = requireTenant(ctx.user);
      const { data } = await db.listProducts(tenantId, undefined, "produto", 1, 100);
  ...
  audit: router({
    list: adminProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        return db.listAuditLogs(tenantId, page, limit, filters);
      }),
  }),
  ...
  reports: router({
    ...
    financialSummary: protectedProcedure
      .input(z.object({ dateFrom: z.date(), dateTo: z.date() }))
      .query(async ({ input, ctx }) => db.getFinancialSummary(requireTenant(ctx.user), input.dateFrom, input.dateTo)),

    clientStatement: protectedProcedure
      .input(z.object({ clientId: z.number() }))
      .query(async ({ input, ctx }) => db.getClientStatement(requireTenant(ctx.user), input.clientId)),
