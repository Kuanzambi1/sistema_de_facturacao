...
describe("invoices router", () => {
  ...
  it("creates proforma invoice (PP) without checking limit or stock", async () => {
    dbMock.getSeriesById.mockResolvedValue({ id: 2, code: "PP2026", year: 2026, validationCode: "DEMO-PP" });
    dbMock.incrementSeriesNumber.mockResolvedValue(1);
    dbMock.getPreviousInvoiceHash.mockResolvedValue(null);
    dbMock.createInvoice.mockResolvedValue({ id: 10, fullNumber: "PP2026/1", documentType: "PP" });
    const { ctx } = createMockCtx();
    const result = await appRouter.createCaller(ctx).invoices.create({
      seriesId: 2, documentType: "PP", issueDate: new Date(),
      items: [{ description: "Computador Portátil", quantity: 2, unitPrice: 250000, vatRate: 14 }],
    });
    expect(result.documentType).toBe("PP");
    expect(dbMock.countMonthlyDocuments).not.toHaveBeenCalled();
    expect(dbMock.applyStockMovementsForInvoice).not.toHaveBeenCalled();
  });

  it("converts proforma (PP) into definitive invoice (FT) with stock movement and AGT submission", async () => {
    dbMock.getTenant.mockResolvedValue({ id: 1, plan: "pro", status: "ativo" });
    dbMock.countMonthlyDocuments.mockResolvedValue(5);
    dbMock.convertQuotation.mockResolvedValue({
      id: 20,
      fullNumber: "FT2026/5",
      documentType: "FT",
      totalAmount: "500000",
      status: "emitida",
      relatedInvoiceId: 10,
      relatedInvoiceNumber: "PP2026/1",
    });
    dbMock.getInvoiceItems.mockResolvedValue([
      { id: 1, productId: 1, description: "Computador Portátil", quantity: "2", unitPrice: "250000", vatRate: "14", total: "500000" }
    ]);
    const { ctx } = createMockCtx();
    const result = await appRouter.createCaller(ctx).invoices.convertQuotation({ quotationId: 10, seriesId: 1 });
    expect(result.documentType).toBe("FT");
    expect(result.fullNumber).toBe("FT2026/5");
    expect(dbMock.convertQuotation).toHaveBeenCalledWith(1, 10, 1, 1);
    expect(dbMock.applyStockMovementsForInvoice).toHaveBeenCalledWith(expect.objectContaining({
      tenantId: 1,
      invoiceId: 20,
      documentType: "FT",
      reference: "FT2026/5",
    }));
    expect(dbMock.addAuditLog).toHaveBeenCalledWith(1, expect.objectContaining({
      action: "converter",
      entityType: "invoice",
      entityId: 20,
      entityLabel: "FT2026/5",
    }));
  });
});
