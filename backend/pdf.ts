...
export async function generateInvoicePdf(invoice: InvoiceData, company: CompanyData): Promise<Buffer> {
  ...
  if (company.logoUrl) {
    try {
      if (company.logoUrl.startsWith("data:")) {
        doc.addImage(company.logoUrl, "PNG", mx, y - 10, 18, 18);
        logoH = 20;
      } else {
        ...
        if (resp.ok) {
          ...
          const ext = company.logoUrl.split(".").pop()?.toLowerCase() ?? "png";
          const mime = ext === "jpg" ? "jpeg" : ext;
          doc.addImage(`data:image/${mime};base64,${buf.toString("base64")}`, "PNG", mx, y - 10, 18, 18);
          logoH = 20;
        }
      }
