...
// ─── Orçamentos & Facturas Proforma → Facturas ───────────────────────────────

export async function convertQuotation(tenantId: number, quotationId: number, seriesId: number, userId: number) {
  const db = await getDb();
  if (!db) return null;
  const { incrementSeriesNumber, getSeriesById, getPreviousInvoiceHash } = await import("./series");
  const { generateDocumentHash, getHashControl } = await import("../fiscal");

  const quote = await getInvoiceById(tenantId, quotationId);
  const isConvertible = quote && (quote.documentType === "OR" || quote.documentType === "PP" || quote.documentType === "FP");
  if (!quote || !isConvertible) throw new Error("Documento não é um orçamento ou factura proforma");
  if (quote.status === "convertida") throw new Error("Documento já convertido");
  if (quote.status === "anulada" || quote.status === "expirada") throw new Error("Documento anulado/expirado");

  const series = await getSeriesById(tenantId, seriesId);
  if (!series) throw new Error("Série não encontrada");
  if (series.documentType !== "FT") throw new Error("A série de destino deve ser do tipo Factura (FT)");
...
export const convertProforma = convertQuotation;
...
export async function deleteInvoice(tenantId: number, invoiceId: number) {
  const db = await getDb();
  if (!db) return null;
  const invoice = await getInvoiceById(tenantId, invoiceId);
  if (!invoice) return null;
  await db.delete(payments).where(and(eq(payments.tenantId, tenantId), eq(payments.invoiceId, invoiceId)));
  await db.delete(invoiceItems).where(and(eq(invoiceItems.tenantId, tenantId), eq(invoiceItems.invoiceId, invoiceId)));
